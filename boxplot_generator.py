...
def boxplot_generator(dframe, outliers=True): # input dframe(DataFrame), outlier(boolean)
    ...
    if outliers == True:
        ax.set(xlabel='Days from Start of Treatment', ylabel='Area (mm^2)', title='Test vs Control with Outliers')
        plt.savefig('{} figure with outliers.png'.format(last_day), dpi=300)
    else:
        ax.set(xlabel='Days from Start of Treatment', ylabel='Area (mm^2)', title='Test vs Control w/o Outliers > {}'.format(x))
